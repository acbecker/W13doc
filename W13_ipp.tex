...
-- calexp science exposures 
-- a database with calexp quality information 
-- selection criteria for coadd
-- coadd type 
...
common zero point.  Multiplicative scale factor applied to the {\tt
  coaddTempExp} vary smoothly across the patch. A two--dimensional
...
%%% HOW BIG A GRID
positions to account for spatial variation of the Psfs.  The model and
idealized Psfs are images in KernelCandidate and part of a
...
The coadded template image is constructed using background matching
images, as opposed to background subtraction images. The assembleCoadd
...
-- Establish use of image variance or per pixel variance for the detection threshold.
...
The metrics will be derive from the mean and root--mean--square values of: the diffim
